...
parser = argparse.ArgumentParser(
    description='Edge Impulse => Edgelab PFLD Meter')
...
print('Transforming Edge Impulse data format into something compatible with Edgelab PFLD Meter')
...
def convert(X, Y, category):
    ...
    for ix in range(0, len(X)):
        raw_img_data = (np.reshape(
            X[ix], (image_width, image_height, image_channels)) * 255).astype(np.uint8)
        ...
        for l in labels:
            ...
            labels_text.append(' {} {}'.format(x, y))

        with open(os.path.join(out_dir, category, "annotations.txt"), 'a') as f:
            f.write("{} {}\n".format(category  + '/images/image' + str(ix).zfill(5) + '.jpg', ' '.join(labels_text)))
...
print('Transforming Edge Impulse data format into something compatible with Edgelab PFLD Meter OK')
print('')
