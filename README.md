# Edgelab PFLD Meter transfer learning model for Edge Impulse

This repository is an example on how to [bring your own model](https://docs.edgeimpulse.com/docs/adding-custom-transfer-learning-models) into Edge Impulse. This repository is using EdgeLab PFLD Meter (an meter reading detection model).
...
1. Convert the training data / training labels into Edgelab PFLD Meter format using [extract_dataset.py](extract_dataset.py).
1. Train PFLD Meter model (using https://github.com/Seeed-Studio/Edgelab).
1. Convert the PFLD Meter model into TFLite format.
1. Done!
...
10. Run the container to test the script (you don't need to rebuild the container if you make changes):
    ...
    ```
    $ docker run --shm-size=1024m --rm -v $PWD:/scripts pfld --data-directory data/ --epochs 30 --learning-rate 0.00001 --out-directory out/
    ```
